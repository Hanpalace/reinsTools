...
👋 欢迎使用 **再保运维工具**！这是一个专为小白和全栈开发者设计的现代化桌面应用模板。无论你是使用 macOS 还是 Windows，都可以通过本项目快速构建出一个美观、功能强大的桌面软件。

📖 **文档**：[使用文档](docs/使用文档.md)（操作手册）· [功能与实现说明](docs/功能说明.md)（功能目录与技术细节）
...
*   🛠️ **系统级能力**: 已封装 WiFi 控制、蓝牙设置、系统通知、开机自启等系统底层功能。
...
pnpm tauri dev
```
🎉 **成功！** 你应该能看到一个名为 "再保运维工具" 的应用窗口弹出来了。
...
│   ├── stores/          # 状态管理 (Pinia)
│   ├── sys-methods/     # 系统底层方法封装 (网络、蓝牙、通知等)
│   ├── views/           # 页面文件
│   ├── App.vue          # 根组件
│   └── main.ts          # 入口文件
